...
void *thread_new_connection(void *client_data)
{
    ...
    file_fd = open(SOCK_FILE_WRITE, O_RDWR);
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (!memcmp(client_node->malloc_wr_buffer,ioc_seek, 19))
            {
                wr_cmd = memchr(client_node->malloc_wr_buffer, ':', malloc_buffer_len);

                if ((client_node->malloc_wr_buffer - wr_cmd) >= (malloc_buffer_len - 1))
                {
                    free(client_node->malloc_wr_buffer);
                    client_node->malloc_wr_buffer = NULL;
                    close(file_fd);
                ...
                wr_offset = memchr(client_node->malloc_wr_buffer, ',', malloc_buffer_len);

                if ((client_node->malloc_wr_buffer - wr_offset) >= (malloc_buffer_len - 1))
                {
                    free(client_node->malloc_wr_buffer);
                    client_node->malloc_wr_buffer = NULL;
                    close(file_fd);
                ...
                *wr_offset = '\0';
                ...
                seekto.write_cmd = atoi(wr_cmd);

                wr_offset++;

                if (wr_offset == client_node->malloc_wr_buffer + (malloc_buffer_len - 1))
                {
                    free(client_node->malloc_wr_buffer);
                    client_node->malloc_wr_buffer = NULL;
                    close(file_fd);
                ...
                client_node->malloc_wr_buffer[malloc_buffer_len - 1] = '\0';

                seekto.write_cmd_offset = atoi(wr_offset);
                syslog(LOG_INFO, "ioctl %d", ioctl(file_fd, AESDCHAR_IOCSEEKTO, &seekto));
        ...
        while (1)
        {
            ret_status = read(file_fd, buff, MAXDATASIZE);

            if (ret_status <= 0)
                break;

            ret_status = write(client_node->fd, buff, ret_status);
            if (ret_status < 0)
            {
                break;
            }
